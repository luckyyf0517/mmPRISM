...
class ManoLayer(Module):
    def __init__(self, manoPath, center_idx=9, use_pca=True, new_skel=False):
        ...
        self.register_buffer('J_regressor', torch.sparse_coo_tensor(i.t(), v, torch.Size([16, 778])).to_dense(), persistent=False)
        # 16 * 3
    ...
    def axis2Rmat(self, axis):
        ...
        rotation_mat = rotation_mat.reshape(-1, 15, 3, 3)
        return rotation_mat
    ...
    def forward(self, root_rotation, pose, shape, trans=None, scale=None):
        ...
        SE3_v = torch.matmul(self.weights, SE3_j.reshape(bs, 16, 16)).reshape(bs, -1, 4, 4)  # bs * 778 * 4 * 4
