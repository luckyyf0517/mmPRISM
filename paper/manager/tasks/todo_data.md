...
| `DATA-003-B` | P1 | CSL-News source adapter 和 manifest | in_progress | cumulative registry/atomic audit/worker whitelist 已验证；17 archive/27,975 videos passed；2,157-record pose+caption partial manifest/adapter 已验收，待 replacement 和最终 436-archive CRC/coverage/decode report |
...
现由 5 分钟 timer 增量维护，`17:30Z` 为 17 个 archive/27,975 videos passed；4 个 dynamic worker 仅消费
