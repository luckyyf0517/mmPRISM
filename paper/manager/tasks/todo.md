...
| `EXP-001` | 建立 canonical OmniHand/WaveLLM 最小 vertical smoke | `DATA-001`, `OPS-001`, `ARCH-001` | in_progress | mT5 geometry-fusion 工程 smoke 已通过；待 OmniHand 与真实 manifest 上的完整 WaveLLM smoke/eval artifact |
...
| `DATA-003` | 为各数据族生成只读 source manifest | `DATA-002` | in_progress | CSL-News registry 为 19 archive/31,423 videos passed，2,157-record pose+caption partial manifest 已验收；`001/005/008` 隔离；待 replacement 和 436-archive complete manifest |
