...
| `DATA-CSLNEWS-INTEGRITY-001` | P0 | `005/008` member 解压损坏；`001` 因 aria2 403 后被旧脚本误晋升为 incomplete final | `005/008` 的 3,251 个视频及 `001` 均不可作为 source；旧 partial snapshot 不能作为 integrity-verified 输入 | active_mitigated | promotion + cumulative consumption gate 已运行；21:24Z 仅调度 registry 中 48 个 passed archive；保留并 versioned 重下 `001/005/008` |
| `DATA-CSLNEWS-ARTIFACT-001` | P1 | GalaxyFS 可见性窗口曾使 `archive_006/3af7...` sidecar 写入空文件 SHA/size，而已晋升 NPZ 实际为 813,674 B | 静默接受会破坏 pose manifest identity；覆盖会销毁 incident evidence | active_mitigated | 9,519-pair clean audit 证明唯一异常；原 pair/失败 snapshot/records 保留，manifest 仅通过 checksum-bound exclusion 隔离；最终 build 前重复全量 audit |
| `EXP-TEST-001` | P1 | legacy 无自动化测试；canonical 已覆盖 foundation、radar、OmniHand/WaveLLM 工程切片和两条 clean-commit GPU formal 闭环 | 真实数据、恢复训练与分布式链仍缺保护 | in_progress | 扩展真实 manifest、resume 和 distributed integration tests |
