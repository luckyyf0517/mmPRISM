...
| `DATA-CSLNEWS-INTEGRITY-001` | P0 | `005/008` member 解压损坏；`001` 因 aria2 403 后被旧脚本误晋升为 incomplete final | `005/008` 的 3,251 个视频及 `001` 均不可作为 source；旧 partial snapshot 不能作为 integrity-verified 输入 | active_mitigated | promotion gate 已修复，`002` 为首个通过新 gate 的 final；仅调度 10 个 audited-pass archive；保留并 versioned 重下 `001/005/008` |
