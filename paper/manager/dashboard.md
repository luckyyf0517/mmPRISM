...
- `/mnt/gfs/yanyifan` 当前未发现 mmPRISM、CSL、OmniHand 或 WaveLLM 数据目录；完整上传清单和
  P0/P1/P2 intake gate 已建立。
- `/mnt/gfs` 当前仅余约 141 GB，首批只能优先接收匿名 metadata、雷达配置/标定和不可替代私人 raw。
...
3. 先提供待上传 archive/目录的名称、大小和可重下标记，再分批接收历史 mmPRISM 数据与证据。
...
2. 按 `data_upload_checklist.md` 执行 `DATA-001-F`：先收集来源端大小清单，不直接移动或解压数据。
