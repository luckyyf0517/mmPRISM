...
| `DEC-043` | 2026-08-12 | 归档并清理 CSL-News 原始下载与运行缓存，保留完成的姿态标注证据 | CSL-News 下载/标注开销不再符合本轮返修重点；CSL-Daily 与新真实语义 CSL 采集更直接支撑修订实验 | accepted | 禁用 CSL-News 计时器并删除 ZIP、partial、labels、cache、live source registry 与 ZIP-dependent source manifest；保留 NPZ/JSON sidecars、QC/reviews、failure evidence、pose manifests/splits，仅可作为 checkpoint-side 预训练/取证候选，不能作为 active source 或论文数据主张 |
| `DEC-044` | 2026-08-12 | 返修固定使用投稿时基于 CSL-News 前约 100 archives 训练的 cam-pose WaveLLM checkpoint，不把 436-archive 重训列为 P0 | 审稿原文没有要求扩大 CSL-News 预训练；核心意见集中在 synthetic-real fidelity、真实数据、域适配和公平架构比较。固定共享初始化能隔离 sim2real 变量；Uni-Sign 的随机比例消融只说明更多数据可能提高绝对上限，不能把顺序前 100 archives 等同于随机 25% | accepted | 先将旧 checkpoint 作为 immutable `original_submission_checkpoint` 恢复文件、SHA-256、mT5/tokenizer/结构、历史 pose/caption/sample/split 和已有评测；所有适用的架构、DA、stress 与 sim2real 对照共享该语义初始化并登记同一 ID/hash。完整 436-archive source intake、标注和重训均不阻塞返修，只有 checkpoint 无法加载/审计、存在 split 泄漏、与必要 pose 契约不兼容，或受控评测确认它成为下游瓶颈时才恢复该工作 |
