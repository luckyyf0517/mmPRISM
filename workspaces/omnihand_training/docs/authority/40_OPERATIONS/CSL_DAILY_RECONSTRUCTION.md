...
This operation consumes only an accepted, immutable CSL-Daily skeleton-simulation pose-reconstruction delivery. It
trains CubeNet to reconstruct its selected camera-pose target from the simulated cube, then exports predictions and
frame features to the WaveLLM lane. It is a synthetic control, not evidence of reconstruction from real measured
radar or of the manuscript's MANO/ray-tracing pipeline.
...
Before a formal run, require a `DELIVERY-POSE-RECON-V1` receipt with source/annotation/simulation provenance,
manifest and split hashes, Parquet inventory/checksums, coordinate frame/units, reader parity, capacity report, and
one-batch GPU adapter smoke. Legacy path JSON and live sidecars are not trainable inputs.
...
```text
accepted synthetic delivery
-> small CubeNet train/reload/evaluate smoke
-> frozen formal CubeNet protocol and checkpoint(s)
-> predicted pose + frame-feature export
-> fold-bound cross-fitted export for WaveLLM training rows
-> WaveLLM consumer receipt
```
...
For a primary predicted-mmWave-pose WaveLLM experiment, a training-row prediction/feature comes from a checkpoint
whose training membership excludes that row. Validation/test outputs come from a checkpoint trained only on the
corresponding training partition. Export coverage must be exactly one valid producer per consumer row. In-sample
exports may support debugging only under `diagnostic_in_sample` and cannot populate comparison tables.
...
The WaveLLM fusion comparison must use the same predicted poses for its pose-only and feature-fusion rows, so the
feature effect is not confounded with a different CubeNet checkpoint or split.
