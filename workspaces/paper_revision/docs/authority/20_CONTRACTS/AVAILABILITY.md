...
| Model/evaluation assets | SBERT/SimCSE 下载、supported model matrix、checkpoint provenance | SBERT/SimCSE ready；mT5-only generation boundary 固定，legacy Phi-3 public support 已移除；mT5/checkpoints 仍待 vertical slice |
...
1. `PAPER-AUDIT-001` 已从 19 个 environment 识别并登记 20 个独立 display item：主文 9 个、
   supplementary 11 个，见 [display item registry](DISPLAY_ITEM_REGISTRY.md)。
