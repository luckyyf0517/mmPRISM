...
| `MODEL-MT5-CSLNEWS-HISTORICAL-V1` | local CSL-News-derived mT5-only language initialization | read-only mirror export; formal destination pending local-derived receipt | six-file inventory verified; `pytorch_model.bin` SHA-256 `6bab01d4...`; differs from official mT5-base; historical parent checkpoint claim unverified | historical/private derivative; redistribution review pending | initialize only canonical language backbone; verify and materialize immutable receipt before formal runs; train pose/radar/fusion adapters anew | smoke_verified_pending_receipt |
| `MODEL-WAVELLM-CAMPOSE-ORIGINAL` | original-submission complete cam-pose WaveLLM semantic initialization | unavailable | full checkpoint, historical hand-pose encoder, config/split/pose contract/predictions absent; no inferred checksum | private paper asset | do not claim loading, reproduction, compatibility, evaluation, or historical metrics; preserve only recoverable script provenance | unavailable_not_a_revision_blocker |
