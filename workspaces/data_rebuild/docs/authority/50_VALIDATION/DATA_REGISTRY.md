...
| `DATASET-CE-CNSL` | CE-CNSL / CE-CSL | official TFNet repository and linked full-video download; versioned local intake pending | publicly downloadable; explicit dataset/repository license and derivative redistribution permission unresolved | full archive reported approximately 9.85 GB; 5,988 RGB videos / 10.52 h | official train/dev/test CSV labels audited locally; immutable source manifest pending | video/CSV/checksum receipt, old signer-label repair, reversible Gloss policy, codec metadata, and 120--240-sequence pose pilot pending | P1 vocabulary and heterogeneous-domain expansion after CSL-Daily baseline; synthetic control only, never real-radar evidence | planned_parallel_intake |
...
| `SPLIT-CE-CNSL-OFFICIAL` | `DATASET-CE-CNSL` | official sample-number split; all 12 published signer labels occur in every split | upstream train/dev/test assignment | 4,973 / 515 / 500 | pending source receipt | sentence/Gloss duplicates audited; signer identity repair still required; not participant-disjoint | pending_intake |
