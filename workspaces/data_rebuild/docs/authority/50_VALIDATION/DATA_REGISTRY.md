...
| `DATASET-CE-CNSL` | CE-CNSL / CE-CSL | official TFNet repository and linked full-video download; no local intake scheduled | publicly downloadable; explicit dataset/repository license and derivative redistribution permission unresolved | full archive reported approximately 9.85 GB; 5,988 RGB videos / 10.52 h | official train/dev/test CSV labels audited locally; immutable source manifest not started | literature/label audit retained; intake, signer repair, reversible Gloss policy, codec audit, and pose pilot deferred until CSL-Daily stable-loop acceptance plus coordinator reactivation | P1 follow-on vocabulary and heterogeneous-domain expansion; synthetic control only, never real-radar evidence | paused_after_csl_daily |
