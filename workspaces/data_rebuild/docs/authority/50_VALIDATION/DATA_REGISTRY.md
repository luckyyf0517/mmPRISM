...
|---|---|---|---|---|---|---|---|---|
| `DATASET-CSL-DAILY` | CSL-Daily | preserved expanded frames at `external/csl_daily/csl_daily_original_20260812/CSL-Daily/sentence/frames_512x512/`; legacy descriptors under `dataset/csl-daily/` | restricted/unknown | 20,654 image sequences; legacy JSON approximately 3.4 MB | legacy `train/val/test/all.json` present only as historical path metadata; `val.json` and `test.json` are byte-identical | v1 pilot is diagnostic-only; P0 v2 GPU smoke, full-corpus annotation QC, and a distinct control split are pending. Per-artifact source root/ID, frame list/count, config/model identity and Git state are recorded; full receipt is optional archive work | camera-pose/skeleton-simulation control and SLU; never a real-radar generalization substitute | reconstruction_ready |
...
| `DATASET-CE-CNSL` | CE-CNSL / CE-CSL | official TFNet repository and linked full-video download; local intake awaits explicit project-owner authorization in CSL-Daily's late stable phase | publicly downloadable; explicit dataset/repository license and derivative redistribution permission unresolved | full archive reported approximately 9.85 GB; 5,988 RGB videos / 10.52 h | official train/dev/test CSV labels audited locally; immutable source manifest not started | literature/label audit retained; an explicit owner authorization may unlock download/receipt only; signer repair, reversible Gloss policy, codec audit, adapter, and pose pilot remain deferred until CSL-Daily stable-loop acceptance plus explicit reactivation | P1 follow-on vocabulary and heterogeneous-domain expansion; synthetic control only, never real-radar evidence | paused_awaiting_optional_download_authorization |
...
|---|---|---|---|---|
| `DELIVERY-POSE-RECON-V1` | `mmprism.pose_reconstruction.sample_v1` | eligible manifest, split, radar/calibration evidence | persisted radar cube `[T,D,R,A,E]`, frame mask, metric `[2,24,3]` pose and validity | direct-cube engineering prototype fixture-verified; prohibited for formal CSL-Daily by `DEC-049`; real calibrated delivery remains blocked |
| `DELIVERY-CSLDAILY-RAW-FMCW-V1` | pending versioned raw-radar pose-reconstruction schema | accepted CSL-Daily source/annotation, frozen split, synthetic FMCW and simulator/processor receipts | persisted `synthetic_fmcw [T,C,A,S]` plus pose/masks; runtime derives transient `[T,D,R,A,E]` power cube | contract/reader/materializer/runtime adapter pending; required before CSL-Daily formal OmniHand run |
...
| `MODEL-MT5-CSLNEWS-HISTORICAL-V1` | local CSL-News-derived mT5-only fallback language initialization | read-only mirror export; formal destination pending local-derived receipt | six-file inventory verified; `pytorch_model.bin` SHA-256 `6bab01d4...`; differs from official mT5-base; historical parent checkpoint claim unverified | historical/private derivative; redistribution review pending | initialize only canonical language backbone if incoming historical bundle audit does not yield an accepted alternative; verify and materialize immutable receipt before formal runs | smoke_verified_pending_receipt |
| `MODEL-WAVELLM-HISTORICAL-BUNDLE-20260812` | staged historical WaveLLM run/checkpoint bundle | `/mnt/gfs/yanyifan/mmPRISM/log/archived/`; staged read-only | four candidate run directories and DeepSpeed-style rank-local state observed; inventory, SHA-256, completeness, model/config/split/metric linkage all pending | private historical paper assets | preserve inbound source unchanged; no load, conversion, move, deletion, or claim before stable receipt and controlled audit | staged_preservation_only |
| `MODEL-WAVELLM-CAMPOSE-ORIGINAL` | original-submission complete cam-pose WaveLLM semantic initialization | candidate may be present within incoming bundle; identity unverified | no accepted checksum, config/split/pose contract/predictions or manuscript linkage yet | private paper asset | resolve only through `MODEL-WAVELLM-HISTORICAL-BUNDLE-20260812` receipt; do not claim loading, reproduction, compatibility, evaluation, or historical metrics before acceptance | candidate_pending_intake_audit |
...
|---|---|---|---|---|
| `CONTRACT-POSE-RECONSTRUCTION-V1` | required `radar_cube [T,D,R,A,E] float32`, `pose_gt [2,24,3] float32`; optional `frame_mask [T] bool`, `pose_valid [2,24] bool` | sample `mmprism.pose_reconstruction.sample_v1`; cube `mmprism.radar_cube.power_v1`; pose unit `m` and explicit coordinate frame | strict relative `.npy` URI、shape/dtype/SHA-256、finite/non-negative power、manifest-wide spatial/coordinate consistency、variable-time zero-padding/masking tests passed | direct-cube adapter_ready；not admissible for formal CSL-Daily after `DEC-049` |
| `CONTRACT-CSLDAILY-RAW-FMCW-V1` | pending `synthetic_fmcw [T,C,A,S]`, `pose_gt [2,24,3]`, masks and simulator/processor references | persistent pre-beamforming FMCW plus runtime `range -> Doppler -> beamforming` conversion | must verify signal encoding/dtype/shape/checksum, simulator and processor fingerprints, CPU/GPU processor parity, no persisted cube, and loader/Parquet reader parity | required_pending |
...
| `SPLIT-CE-CNSL-OFFICIAL` | `DATASET-CE-CNSL` | official sample-number split; all 12 published signer labels occur in every split | upstream train/dev/test assignment | 4,973 / 515 / 500 | no source manifest yet | sentence/Gloss duplicates audited; signer identity repair still required; not participant-disjoint | paused_with_dataset |
