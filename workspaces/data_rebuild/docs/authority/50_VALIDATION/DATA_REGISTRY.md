...
| `MODEL-WAVELLM-HISTORICAL-BUNDLE-20260812` | incoming historical WaveLLM run/checkpoint bundle | `/mnt/gfs/yanyifan/mmPRISM/log/archived/`; upload in progress | four candidate run directories observed; first partial candidate has DeepSpeed-style rank-local state; inventory, SHA-256, completeness, model/config/split/metric linkage all pending | private historical paper assets | preserve inbound source unchanged; no load, conversion, move, deletion, or claim before stable receipt and controlled audit | transfer_in_progress_preservation_only |
