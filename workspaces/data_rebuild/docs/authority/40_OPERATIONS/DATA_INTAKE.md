...
[workspace index](../00_INDEX.md) 的当前状态或 [data registry](../50_VALIDATION/DATA_REGISTRY.md)
的 provenance 登记。
...
`CC BY-NC 4.0`，436 archives 总计约 935 GB compressed。若未来明确批准完整重训，必须从 data registry
复核当前 source availability，并在缺失时重新建立 versioned intake 和 source manifest；不能把 partial pose
artifacts 当作完整 source 状态。
...
|---|---|---|
| Language backbone | `google/mt5-base` 或历史本地 `mt5-pretrained-news` | 公共 base 按固定 revision 下载；本地 mT5 export 按 P0-G receipt 登记，不能标为官方 base；其他历史权重按 P1 |
...
- [ ] [data registry](../50_VALIDATION/DATA_REGISTRY.md) 已登记
  owner/location/size/access/validation 状态。
...
5. 优先 intake CSL-Daily；CSL-News 仅在明确批准的 future ceiling/provenance experiment 后重新下载。
6. 优先为 P0-G 本地 mT5 export 创建 receipt；容量允许后再上传其他原投稿 checkpoint、predictions、metrics
   和 figure source data。若找回完整 WaveLLM，按 immutable historical evidence intake，而不是改写当前主线。
