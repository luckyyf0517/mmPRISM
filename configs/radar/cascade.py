...
freq_slope = 70e12 
adc_sample_rate = 2.034e6
num_adc_samples = 100

idle_time = 9e-6
adc_start_time = 5e-6  
ramp_end_time = 55.70e-6  
adc_sample_end_time = adc_start_time + num_adc_samples / adc_sample_rate 
assert adc_sample_end_time < ramp_end_time, f"Adc sampling time exceeds chirp duration: {adc_sample_end_time} >= {ramp_end_time}"

num_chirps = 64
frame_period = 0.1
chirp_period = (idle_time + ramp_end_time) * 12
...
doppler_resolution = wave_length / (2 * chirp_period)
